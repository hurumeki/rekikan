...
export function CategoriesTable() {
  ...
  function saveNew() {
    if (!newCatForm?.value || !newCatForm?.label) {
      alert('値とラベルは必須です');
      return;
    }
    if (!/^[a-zA-Z0-9_]+$/.test(newCatForm.value)) {
      alert('値は英数字・アンダースコアのみ');
      return;
    }
    if (state.categories.some((c) => c.value === newCatForm.value)) {
      alert('この値はすでに存在します');
      return;
    }
    dispatch({
                    ...
                    {isEditing ? (
                      <Input
                        ...
                        onChange={(e) =>
                          setEditingValues((p) => ({
                            ...p,
                            [cat.value]: { ...edit, label: e.target.value },
                          }))
                        }
                    ...
                    {isEditing ? (
                      <Input
                        ...
                        onChange={(e) =>
                          setEditingValues((p) => ({
                            ...p,
                            [cat.value]: { ...edit, icon: e.target.value },
                          }))
                        }
      ...
      {newCatForm ? (
        <div className="border border-primary/30 rounded-md p-3 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            <Input
              placeholder="値 (英数字)"
              value={newCatForm.value}
              onChange={(e) => setNewCatForm((p) => (p ? { ...p, value: e.target.value } : p))}
              className="h-8 text-xs"
            />
            <Input
              placeholder="ラベル"
              value={newCatForm.label}
              onChange={(e) => setNewCatForm((p) => (p ? { ...p, label: e.target.value } : p))}
              className="h-8 text-xs"
            />
            <Input
              placeholder="アイコン絵文字"
              value={newCatForm.icon}
              onChange={(e) => setNewCatForm((p) => (p ? { ...p, icon: e.target.value } : p))}
              className="h-8 text-xs"
            />
          </div>
          <div className="flex gap-2">
            ...
            <Button size="sm" variant="outline" onClick={() => setNewCatForm(null)}>
              キャンセル
            </Button>
          </div>
        </div>
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => setNewCatForm({ value: '', label: '', icon: '📌' })}
        >
