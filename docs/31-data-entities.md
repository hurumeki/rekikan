...
| region         | string                                  | ✓        | Primary region ID                                                                                                       |
| regions        | string[] / null                         |          | All involved region IDs for cross-region quizzes. Omitted for regular quizzes                                           |
...
| modes          | string[]                                | ✓        | Available play modes: `"careful"`, `"challenge"`, `"timeline"`, `"era_band"`, `"cross_region"`                          |
| difficulty     | number                                  | ✓        | Difficulty level (1 = easy, 2 = medium, 3 = hard)                                                                       |
| timeline_range | `{ start: number; end: number }` / null |          | Override for the timeline display range (years). If omitted, the range is auto-calculated from the cards' `year` values |
...
```json
{
  ...
  "label": "中世・近世",
  "sort_order": 2,
  "quiz_ids": ["quiz_japan_era_medieval_desc", "quiz_japan_era_medieval"],
  "unlock_condition": {
