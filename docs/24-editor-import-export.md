...
| Image presence         | If a card has `has_image: true`, `public/images/cards/{id}.webp` exists (warning only)                                    |
| Cover image presence   | If a node has `has_cover_image: true`, `public/images/nodes/{id}.webp` exists (warning only)                              |

Image binaries themselves are not included in the export JSON; only the `has_image` / `has_cover_image` flags ride along. The image files under `public/images/` are versioned alongside the rest of the repository.
